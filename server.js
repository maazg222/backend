...
// Middleware
app.use(cors({
  origin: '*', // For production, replace with your actual frontend URL for better security
  methods: ['GET', 'POST', 'PUT', 'DELETE', 'OPTIONS'],
  allowedHeaders: ['Content-Type', 'Authorization', 'x-user-id', 'x-user-rank']
}));
...
// Static assets - Only if not in serverless mode (or if we want to bundle them)
if (!process.env.VERCEL) {
  ...
  // If in Vercel, provide a simple health check or base response
  app.get('/', (req, res) => {
...
// Ping/Health Check endpoint for Vercel
app.get('/api/ping', (req, res) => {
  res.json({ status: 'online', timestamp: new Date().toISOString(), serverless: !!process.env.VERCEL });
});
